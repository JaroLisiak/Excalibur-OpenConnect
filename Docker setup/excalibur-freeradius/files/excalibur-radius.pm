...
use constant RLM_MODULE_HANDLED =>
  3;    #  /* the module handled the request, so stop. */
use constant RLM_MODULE_INVALID =>
  4;    #  /* the module considers the request invalid. */
use constant RLM_MODULE_USERLOCK =>
  5;    #  /* reject the request (user is locked out) */
use constant RLM_MODULE_NOTFOUND => 6;    #  /* user not found */
use constant RLM_MODULE_NOOP =>
  7;    #  /* module succeeded without doing anything */
...
our @CONFIG_FILES = "/etc/freeradius/rlm_perl.ini";
our $Config  = {};
our $Mapping = {};
our $cfg_file;
        ...
        foreach my $member ( $cfg_file->GroupMembers($group) ) {
            ...
            if ( $group eq "Mapping" ) {
                foreach my $key ( $cfg_file->Parameters($member) ) {
                    ...
                    &radiusd::radlog( Info,
                        "++++++ Map: $topnode : $key -> $radiusAttribute" );
                    $radReply{$radiusAttribute} =
                      $decoded->{detail}{$topnode}{$key};
                }
            }
            if ( $group eq "Attribute" ) {
                ...
                &radiusd::radlog( Info,
"++++++ Attribute: IF '$directory'->'$userAttribute' == '$regex' THEN '$radiusAttribute'"
                );
                my $attributevalue = "";
                ...
                if ( ref($attributevalue) eq "" ) {
                    &radiusd::radlog( Info,
                        "+++++++ User attribute is a string: $attributevalue" );
                    push( @values, $attributevalue );
                }
                if ( ref($attributevalue) eq "ARRAY" ) {
                    &radiusd::radlog( Info,
                        "+++++++ User attribute is a list: $attributevalue" );
                ...
                foreach my $value (@values) {
                    ...
                    if ( $value =~ /$regex/ ) {
                        ...
                        &radiusd::radlog( Info,
"++++++++ Result: Add RADIUS attribute $radiusAttribute = $result"
                        );
                    }
                    else {
                        &radiusd::radlog( Info,
"++++++++ Result: No match, no RADIUS attribute $radiusAttribute added."
                        );
                    }
                }
            }
...
sub authenticate {
    ...
    # put RAD_REQUEST members in the back-end request
    ...
    &radiusd::radlog( Info, "state sent to back-end: $params{'state'}" );
    if ( $debug == true ) {
    ...
    # Set the user-agent to be fetched in back-end Client Application Type
    $ua->agent("FreeRADIUS");
    if ( $check_ssl == false ) {
        try {
            ...
            &radiusd::radlog( Error,
                "ssl_opts only supported with LWP 6. error: $@" );
        }
    }
    ...
    $RAD_REPLY{'Reply-Message'} = "Excalibur back-end server denied access!";
    ...
    if ( !$response->is_success ) {
        ...
        &radiusd::radlog( Info, "Excalibur back-end request failed: $status" );
        $RAD_REPLY{'Reply-Message'} = "Back-end request failed: $status";
    ...
    try {
        ...
        if ( $decoded->{result}{value} ) {
            &radiusd::radlog( Info, "Excalibur back-end access granted" );
            $RAD_REPLY{'Reply-Message'} = "Excalibur back-end access granted";
            ...
            &radiusd::radlog( Info,
                "Excalibur back-end Result status is true!" );
            ...
            if ( $decoded->{detail}{transaction_id} ) {
                ...
                &radiusd::radlog( Info, "Excalibur back-end access denied" );
                $g_return = RLM_MODULE_REJECT;
            }
        }
        elsif ( !$decoded->{result}{status} ) {
            &radiusd::radlog( Info,
                "Excalibur back-end Result status is false!" );
        ...
        &radiusd::radlog( Info,
            "Can not parse response from Excalibur back-end." );
    };
